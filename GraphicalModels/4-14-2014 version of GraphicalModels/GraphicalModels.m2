...
export {"bidirectedEdgesMatrix",
       ...
       "verticesInU",
       "verticesInW",
       "permuteVerticesOfGraph",
       "UWDecomposition",
       "checkSullivantTalaskaDraismaConditions"
       	} 
...
------------------------------------------------------------------------------------------------------------------------------------
--The following functions are used to check if a mixed graph is in the form 
--required by Sullivant, Talaska, and Draisma, and if not, to permute the 
--vertices to bring it into the required form.  
------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------
--Checked by Dave, 3/27/2014  Looks good
...
{*
Compute the UW decomposition 
From Sullivant-Talaska-Draisma, 
"Trek separation for Gaussian graphical models"
page 9, Section 2.3.
*}
UWDecomposition=method();

UWDecomposition(MixedGraph) := (G) -> (
    Un := G#graph#Graph;	   
    Bi := G#graph#Bigraph;    
    Di := G#graph#Digraph;
    vertUn := vertices(Un);
    vertBi := vertices(Bi);
    vertDi := vertices(Di);
    if vertUn=={} then return {{},sort vertices(G)};
    if vertBi=={} then return {sort vertices(G),{}};
    flag:=true;
    ed:={};
...
4. the directed edges must form an acyclic graph
*}

checkSullivantTalaskaDraismaConditions=method();

checkSullivantTalaskaDraismaConditions(MixedGraph,List,List) := (G,U,W) -> (
    Un := G#graph#Graph;	   
    Bi := G#graph#Bigraph;    
    Di := G#graph#Digraph;
    vertUn := vertices(Un);
    ...
    --Check condition 4: the directed edges must form an acyclic graph
    if isCyclic(Di) then (
        print "The directed edges must form an acyclic graph";
	return false	
    );
    return true
);
...
gaussianRing = method(Dispatch=>Thing, Options=>{Coefficients=>QQ, sVariableName=>"s", lVariableName=>"l", 
	  pVariableName=>"p", kVariableName=>"k", verticesInU=>null})
...
--     );
     ...
     vv := sort vertices g;  
     U:={};
     if instance(opts.verticesInU,List) then U=opts.verticesInU;
     if instance(opts.verticesInU,Nothing) then (
         UW := UWDecomposition(g);
         U=UW_0;
     );   
     W:=drop(vv,{0,#U-1});
     b:=checkSullivantTalaskaDraismaConditions(g,U,W);
     ...
     k := toSymbol opts.kVariableName;     
     kk := opts.Coefficients;          
     if (not gaussianRingList#?(kk,s,l,p,k,g,U,W)) then ( 
     ...
     pL := {};
     kL := {};
     if #W > 0 then pL = join(apply(W, i->p_(i,i)),delete(null, flatten apply(vv, x-> apply(toList bb#x, y->if pos(vv,x)>pos(vv,y) then null else p_(x,y)))));
     if #U > 0 then kL = join(apply(U, i-> k_(i,i)),delete(null, flatten apply(vv, x-> apply(toList uu#x, y->if pos(vv,x)>pos(vv,y) then null else k_(x,y)))));
     m := #lL+#pL+#kL;
     ...
     R.mixedGraph = g;
     gaussianRingList#((kk,s,l,p,k,g,U,W)) = R;); 
     gaussianRingList#((kk,s,l,p,k,g,U,W))
     )
...
undirectedEdgesMatrix Ring := Matrix =>  R -> (
	...
	H = R.gaussianVariables;
        PM = mutableMatrix(R,#vv,#vv);
        scan(vv,i->PM_(pos(vv,i),pos(vv,i))=H#(k_(i,i)));
        scan(vv,i->if uu#?i then scan(toList uu#i, j->PM_(pos(vv,i),pos(vv,j))=if pos(vv,i)<pos(vv,j) and H#?(k_(i,j)) then H#(k_(i,j)) else if pos(vv,i)>pos(vv,j) and H#?(k_(j,i)) then H#(k_(j,i))));
        return matrix PM
    );
    if R.?mixedGraph then (
	g = R.mixedGraph;
	uu = graph(g#graph#Graph);
	vv = R.verticesInU;
	n = R.gaussianRingData#0;
	k = R.gaussianRingData#4;
	H = R.gaussianVariables;
        PM = mutableMatrix(R,#vv,#vv);
        scan(vv,i->PM_(pos(vv,i),pos(vv,i))=H#(k_(i,i)));
        scan(vv,i->if uu#?i then scan(toList uu#i, j->PM_(pos(vv,i),pos(vv,j))=if pos(vv,i)<pos(vv,j) and H#?(k_(i,j)) then H#(k_(i,j)) else if pos(vv,i)>pos(vv,j) and H#?(k_(j,i)) then H#(k_(j,i))));
...
gaussianParametrization Ring := Matrix => opts -> R -> (
    if not R.?gaussianRingData then error "expected a ring created with gaussianRing";     
    if not R.?mixedGraph then error "currently only implemented for a gaussianRing of a mixed graph";
    ...
    vertDi := vertices(Di);
    K := frac R;
    S := substitute(covarianceMatrix R,K);
    U := substitute(undirectedEdgesMatrix R, K);
    Uinv := inverse(U);
    W := substitute(bidirectedEdgesMatrix R, K);
    L := substitute(directedEdgesMatrix R, K);
    Li := inverse(1-matrix(L));    
    M := matrix 0;
    n := R.gaussianRingData#0;
    if #(R.verticesInU) >0 and #(R.verticesInW) >0 then (
	Z12 := matrix apply(numRows(Uinv), i -> apply(n-numColumns(Uinv),j -> 0_K));
        Z21 := matrix apply(numRows(W), i -> apply(n-numColumns(W),j -> 0_K));
	M= ((Uinv|Z12)||(Z21|W))
    );
    if #(R.verticesInU) == 0 and #(R.verticesInW) >0 then M=W;
    if #(R.verticesInU) > 0 and #(R.verticesInW) == 0 then M=U;
    --If there are bidirected edges, need to invert M
    if #vertBi >= 0 then (
         M = transpose(Li)*M*Li;
    ...
    --To do: If there are no bidirected edges, no Psi, parametrize concentration matrix instead of covariance matrix
    --Not implemented yet
       )
...
identifyParameters Ring := HashTable => R -> (
     ...
     if R.verticesInU != {} then error "Currently only implemented for mixed graphs with no undirected edges"; 
     g := R.mixedGraph;
     J := ideal unique flatten entries (covarianceMatrix(R)-substitute(gaussianParametrization(R),R));
     G := graph g;
...
doc ///
  ...
  Description
    Text 
      Here is a link to what's new in @TO "GraphicalModels version 1.2"@
    Text
...
doc ///
    Key
        "GraphicalModels version 1.2"
    Headline
        What's new in version 1.2
    Description
    
        Text
            We updated gaussianRing and are in the process of making GraphicalModels compatible with the new Graphs package.
	Text    
	    The Gaussian ring of a graph acts just like before.
        Example
	    gG=graph {{1,3},{2,4}}
            RG=gaussianRing(gG);
	    print toString gens RG
            undirectedEdgesMatrix(RG)
        Text
	    Note: "directedEdgesMatrix(RG)" and "bidirectedEdgesMatrix(RG)" and "gaussianParametrization(RG)" will give errors.
	Text
	    Here is an example of a mixed graph that has only undirected edges.
	Example    
            g=graph {{1,3},{2,4}};
            H=new HashTable from {Graph=>g, Digraph=>digraph {}, Bigraph => bigraph {} };
            gG = new MixedGraph from { {graph,H}, {cache,new CacheTable from {}} }
            RG=gaussianRing(gG);
            print toString gens RG
            undirectedEdgesMatrix(RG)
            directedEdgesMatrix(RG)
            bidirectedEdgesMatrix(RG)
            gaussianParametrization(RG)
        Text
	    Here is an example of a digraph.
	Example
            gD = digraph {{1,{2}},{2,{3,4}}};
            RD=gaussianRing(gD);
            print toString gens RD
	Text 
	    "undirectedEdgesMatrix(RD)", "directedEdgesMatrix(RD)", "bidirectedEdgesMatrix(RD)", and "gaussianParametrization(RD)" will give errors.  
        Text
	    Here is an example of a digraph embedded in a mixed graph.  
	Example
	    gD = mixedGraph(digraph {{1,{2}},{2,{3,4}}});
	    RD=gaussianRing(gD);
	    print toString gens RD
	    undirectedEdgesMatrix(RD)
	    directedEdgesMatrix(RD)
	    bidirectedEdgesMatrix(RD)
	    gaussianParametrization(RD)
        Text
            Here is an example of a chain graph (a graph with no bidirected edges).
	Example
            gGD = mixedGraph(graph {{1,3},{2,4}} , digraph {{1,{2}},{2,{3,4}}});
            RGD = gaussianRing(gGD);
            print toString gens RGD
            undirectedEdgesMatrix(RGD)
            directedEdgesMatrix(RGD)
            bidirectedEdgesMatrix(RGD)
            gaussianParametrization(RGD)
        Text
	    Here is an example of a mixed graph with no undirected edges.
	Example
	    gDB = mixedGraph(digraph {{1,{2}},{2,{3,4}}}, bigraph {{1,3},{2,4}})
            RDB=gaussianRing(gDB);
            print toString gens RDB
            undirectedEdgesMatrix(RDB)
            directedEdgesMatrix(RDB)
            bidirectedEdgesMatrix(RDB)
            gaussianParametrization(RDB)
	Text	
	    Here is a mixed graph with all three kinds of edges.
	Example
	    gGDB = mixedGraph(graph {{1,2}},digraph {{1,{2}},{2,{3,4}}},bigraph {{3,4}});
	    RGDB= gaussianRing(gGDB);
	    print toString gens RGDB
	    undirectedEdgesMatrix(RGDB)
	    directedEdgesMatrix(RGDB)
	    bidirectedEdgesMatrix(RGDB)
	    gaussianParametrization(RGDB)
       Text
           The next two examples are examples of a graph with more than one possible UW decomposition in the sense of Sullivant, Talaska, and Draisma.
       Text
           Here is the Gaussian ring that arises from the package's default UW decomposition.
       Example
           gGDB5 = mixedGraph(graph {{1,2}},digraph {{1,{2}},{2,{3,4,5}}},bigraph {{4,5}});
           RGDB5= gaussianRing(gGDB5);
           print toString gens RGDB5
           undirectedEdgesMatrix(RGDB5)
           directedEdgesMatrix(RGDB5)
           bidirectedEdgesMatrix(RGDB5)
           gaussianParametrization(RGDB5) 
       Text
           The user may specify the UW decomposition with an optional input.  
       Example
	   gGDBo = mixedGraph(graph {{1,2}},digraph {{1,{2}},{2,{3,4,5}}},bigraph {{4,5}});
           RGDBo= gaussianRing(gGDBo,verticesInU=>{1,2});
           print toString gens RGDBo
           undirectedEdgesMatrix(RGDBo)
           directedEdgesMatrix(RGDBo)
           bidirectedEdgesMatrix(RGDBo)
           gaussianParametrization(RGDBo)
///;
...
restart
 ...
 installPackage("GraphicalModels", RemakeAllDocumentation => true, UserMode=>true, DebuggingMode => true, FileName => "/Users/lgp/Software/Macaulay2/Workshop-2014-Berkeley/GraphicalModels/4-2-2014 version of GraphicalModels/GraphicalModels.m2")
