...
gaussianRing MixedGraph := Ring => opts -> (g) -> (
     ...
     sigmaG:=orderingOfMixedGraph(g);
     sg:=sigmaG_0;
     perm:=sigmaG_1;
     print concatenate("Permuting the graph by the permutation ",toString(perm)) << endl;
     print concatenate("The reordered graph is ",toString(sg)) << endl;
     numUn:=sigmaG_2;
     numBi:=sigmaG_3;
     G := graph collateVertices sg;
     dd := graph G#Digraph;
     bb := graph G#Bigraph;
     uu := graph G#Graph;    
     vv := sort vertices sg;
     --using the Sullivant-Talaska-Draisma UW decomposition
     verticesInU:=apply(numUn, i -> vv_i);
     verticesInW:=apply(numBi, i -> vv_(i+numUn));
     s := toSymbol opts.sVariableName;
