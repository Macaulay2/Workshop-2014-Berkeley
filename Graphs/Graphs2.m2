-- Copyright 2013: Jack Burkart, David Cook II, Caroline Jansen, Sonja Mapes
...
newPackage select((
    "Graphs",
        ...
        Authors => {
            ...
            -- Augustine O'Keefe (aokeefe@tulane.edu)'
            -- Doug Torrance
            -- Shaowei Lin, Alex Diaz -- contributed mixedGraph
        },
...
digraph = method(Options => {symbol Singletons => null, symbol EntryMode => "neighbors"})
digraph List := Digraph => opts -> L -> (
    if opts.EntryMode == "neighbors" then (
		E := flatten apply(L, N -> apply(N_1, v -> {N_0, v}));
		V := unique join(flatten E, if instance(opts.Singletons, List) then opts.Singletons else {});
		digraph(V, E)
		)
    else if opts.EntryMode == "edges" then digraph(unique flatten (toList \ L), L, opts)
    else error "That is not an option for EntryMode.  Try 'neighbors' or 'edges'."
    )
digraph HashTable := Digraph => opts -> g -> digraph(flatten apply(keys g, v -> apply(g#v, u -> {v, u})), opts)
digraph (List, List) := Digraph => opts -> (V, E) -> (
    E = toList \ E;
    ...
    G := hashTable apply(V, v -> v => select(V, u -> member({v,u}, E)));
    new Digraph from hashTable {
        symbol graph => G,
        symbol vertexSet => V,
...
digraph (List, Matrix) := Digraph => opts -> (V, A) -> (
    V = unique join(V, if instance(opts.Singletons, List) then opts.Singletons else {});
    G := hashTable apply(#V, i -> V_i => V_(positions(first entries A^{i}, j -> j != 0)));
    new Digraph from hashTable {
        symbol graph => G,
        symbol vertexSet => V,
        symbol adjacencyMatrix => A,
...
graph = method(Options => {symbol Singletons => null, symbol EntryMode => "neighbors"})
graph (List, List) := Graph => opts -> (V, E) -> (
    E = toList \ E;
    if not isSubset(flatten E, V) then error "There are edges with vertices outside the vertex set.";
    V = unique join(V, if instance(opts.Singletons, List) then opts.Singletons else {});
    A := if #V == 0 then map(ZZ^0, ZZ^0, 0) else matrix apply(#V, i -> apply(#V, j -> if member({V#i, V#j}, E) or member({V#j, V#i}, E) then 1 else 0));
    G := hashTable apply(#V, i -> V_i => V_(positions(first entries A^{i}, j -> j != 0)));
    new Graph from hashTable {
        symbol graph => G, 
        symbol vertexSet => V, 
        symbol adjacencyMatrix => A,
        symbol cache => new CacheTable from {}}
...
graph List := Graph => opts -> L -> (
    if opts.EntryMode == "neighbors" then graph(hashTable apply(L, x -> x_0 => toList(x_1)), opts)
    else if opts.EntryMode == "edges" then graph(unique flatten (toList \ L), L, opts)
    else error "That is not an option for EntryMode.  Try 'neighbors' or 'edges'."
    )
graph HashTable := Graph => opts -> (g) -> graph(keys g, flatten apply(keys g, v -> apply(g#v, u -> {v, u})), opts)
...
graph Digraph := HashTable => opts -> D -> D.graph
...
degree (Digraph,Thing) := ZZ => (D,v) -> #unique join(children(D,v),parents(D,v))

degreeMatrix = method()
degreeMatrix Graph := Matrix => G -> diagonalMatrix apply(entries transpose adjacencyMatrix G, a -> #positions(a, j -> j != 0))
...
complementGraph (Graph) := Graph => G -> (
	possibleEdges := subsets(#vertexSet G, 2);
	actualEdges := set(toList \ edges G);
	complementEdges := possibleEdges - actualEdges;
	graph(complementEdges, Singletons => vertexSet G - set flatten complementEdges, EntryMode => "edges")
	)

digraphTranspose = method ()
digraphTranspose (Digraph) := Digraph => D -> (
    E := apply(edges D, i -> {last i, first i});
    digraph E
    )

underlyingGraph = method ()
underlyingGraph Digraph := Graph => D -> graph(vertexSet D, edges D)
...
completeGraph ZZ := Graph => n -> graph(toList(0..<n), subsets(n, 2))
...
cycleGraph = method()
cycleGraph ZZ := Graph => n -> graph(toList(0..<n), prepend({0,n-1}, apply(n-1, i -> {i, i+1})))
...
wheelGraph ZZ := Graph => n -> (
    ...
    graph(toList(0..<n), join(spokes,outside))
    )
...
vertexCuts = method()
--so West does not specify but Wikipedia does that K_n has no vertex cuts.  So the method currently returns an empty list, which is technically correct.
...
children = method ()
children(Digraph, Thing) := List => (G,v) -> (
    i := position(vertexSet G, u -> u == v);
    if i === null then error "v is not a vertex of G.";
    (vertexSet G)_(positions(first entries (adjacencyMatrix G)^{i}, j -> j != 0))
    )
...
closedNeighborhood (Graph, Thing) := List => (G,a) -> neighbors(G,a) | {a}
...
degreeCentrality(Graph, Thing) := RR => (G, v) -> (
    degree(G, v)/(2*#edges G)
    )
...
descendants = method () 
descendants(Digraph, Thing) := List => (D,v) -> flatten breadthFirstSearch(D, v)
...
distance = method ()
distance (Graph, Thing, Thing) := ZZ => (G,v,u) -> (
...
distance (Graph, Thing) := HashTable => (G, v) -> (
...
forefathers = method () 
forefathers (Digraph, Thing) := List => (D,v) -> flatten reverseBreadthFirstSearch(D,v)
...
nonneighbors = method()
nonneighbors (Graph, Thing) := (G,v) -> vertexSet G - set prepend(v, neighbors(G, v))
...
parents = method ()
parents (Digraph, Thing) := List => (G,v) -> (   
	i := position(vertexSet G, u -> u == v);
    if i === null then error "v is not a vertex of G.";
    (vertexSet G)_(positions(flatten entries (adjacencyMatrix G)_{i}, j -> j != 0))
    )
...
topologicalSort(Digraph, String) := List => (D,s) -> (
    ...
    if s == "bfs" then (
        ...
        while S != {} do (
            ...
            S = drop (S, 1);
            L = append(L, v);
            S = join(S, select(children (D, v), c -> isSubset(parents(D,c), L)));         
            );    
        return L;
        );
    if s == "random" then (
        ...
        while S != {} do (
            ...
            S = drop (S, 1);
            L = append(L, v);
            S = random join(S, select(children (D, v), c -> isSubset(parents(D,c), L)));         
            );    
        return L;
        );
    if s == "dfs" then (
        ...
        while S != {} do (
            ...
            S = drop (S, 1);
            L = append(L, v);
            S = join(select(children (D, v), c -> isSubset(parents(D,c), L)), S);         
            );    
        return L;
        );
    if s == "minnumerical" then (
        L = {};
        S = sort sources D;
        while S != {} do (
            v = S_0;
            S = drop (S, 1);
            L = append(L, v);
            S = sort join(S, select(children (D, v), c -> isSubset(parents(D,c), L)));         
            );    
        return L;
        );   
    if s == "maxnumerical" then (
        L = {};
        S = rsort sources D;
        while S != {} do (
            v = S_0;
            S = drop (S, 1);
            L = append(L, v);
            S = reverse sort join(S, select(children (D, v), c -> isSubset(parents(D,c), L)));         
            );    
        return L;
        );
    )    
-- TODO: This really should be compressed dramatically.  A lot of this code is reused.
...
isSink = method ()
isSink (Digraph, Thing) := Boolean => (D,v) -> children(D,v) == {}

isSource = method ()
isSource (Digraph, Thing) := Boolean => (D,v) -> parents(D,v) == {}
...
---------------------------
addEdge = method ()
addEdge (Graph, Set) := Graph => (G, s) -> addEdges(G, {toList s})
...
-- TODO: These two can be merged.  Check the instance of G. 
-- If it is a Graph, then run the A_(reverse) line.
...
-- These two should be merged... they're approximately the same.
...
reindexBy (Graph, String) := Graph => (G, s) -> (
    ...
    if s == "random" then return graph (random vertexSet G, edges G);
    if s == "components" then return graph (flatten connectedComponents G, edges G);
    if s == "sort" then return graph (sort vertexSet G, edges G);
    )
reindexBy (Digraph, String) := Digraph => (D, s) -> (
    ...
    if s == "random" then return digraph(random vertexSet D, edges D);
    if s == "sort" then return digraph(sort vertexSet D, edges D);
    )    
...
spanningForest Graph := Graph => G -> (
    ...
    E := {};
    for v in V do (
        N := neighbors (G,v) - set select(V, x -> member(x, flatten E));
        E = E | apply(N, n -> {v,n}); 
        );
    graph(V, E)
    )
...
vertexMultiplication(Graph, Thing, Thing) := Graph => (G,v,u) -> (
    ...
    graph(append(vertexSet G, u), edges G | apply(neighbors (G,v), i -> {i,u}))
    )

--note:  new methods must be documented.
...
doc///    
	...
	Description
		...
		Example
			...
			G = digraph ({a,b,c,d,e},{{a,b},{b,c},{b,d},{e,b}})
...
doc ///
	...
	Description
		...
		Example
			G = graph graph({{1,2},{2,1},{3,1}}, EntryMode => "edges")
			G = graph graph(matrix {{0,1,1},{1,0,0},{1,0,0}})
	SeeAlso
		digraph
///
...
--	Headline
--		returns the degree of a vertex in a graph or digraph
--	Usage
--		x = degree (G,v)
--		x = degree (D,v)
--	Inputs
...
doc ///
	...
	Outputs
		L:List
			a list of all the descendants of v 
	Description
		Text
...
doc ///
	Key
		distance
		(distance, Graph, Thing)
	Headline
		computes the distance between a specifed vertex and all other vertexSet in a graph.
	Usage
		H = distance(G,v)
	Inputs
		G:Graph
	...
	Outputs
		H:HashTable
			the HashTable with keys corresponding to each vertex in the graph and values corresponding to the distance between vertex v and the key vertex
	Description
		Text
			The distance between two vertexSet is calculated as the number of edges in the shortest path between the two vertexSet.  If the two vertexSet are not connected, the distance between them is infinity.
		Example
			G = graph({{1,2},{2,3},{3,4}},EntryMode=>"edges");
			d = distance (G,1)
			G = graph({1,2,3,4},{{2,3},{3,4}});
...
doc ///
	...
	Outputs
		L:List
			a list of all the forefathers of v in D
	Description
		Text
